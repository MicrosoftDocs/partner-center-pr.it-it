...
Questo articolo descrive i vantaggi specifici di Azure e cloud disponibili per i partner dall'interno Partner Center. Si apprenderà anche come attivare determinati tipi di sottoscrizioni di Azure e cloud.
...
- Sono disponibili & di sottoscrizione di prodotti cloud di Azure, indipendentemente dalle sottoscrizioni basate su licenza o dall'utilizzo
- Quantità di licenza assegnata per ogni prodotto
- Tipo di offerta che ha effettuato il provisioning di un prodotto vantaggio specifico all'utente
- Tipo di vantaggio (che sia di base o incrementale)
...
- Quando si assegna il credito di Azure in blocco (annuale), si noti che **l'opzione** Rimuovi non è disponibile in Partner Center. È invece necessario trasferire il credito in blocco a un altro utente durante l'anno di registrazione. La sottoscrizione di credito in blocco (annuale) di Azure non può essere combinata con altre offerte di licenza, prepagata o di credito (ad esempio Microsoft Contratto Enterprise, Cloud Solution Provider, Azure in Open, Azure Pass, Microsoft Action Pack e così via).

### <a name="license-based-subscriptions"></a>Abbonamenti in base alle licenze

Nella sezione sottoscrizioni basate su licenza della pagina Prodotti Azure e Cloud sono disponibili tutti i prodotti cloud che richiedono un codice Product Key per l'attivazione, ad esempio Microsoft O365, Microsoft Dynamics, Microsoft Intune, Azure AD Basic, Power BI e così via. Questi prodotti vengono offerti in base al tipo di offerta attiva disponibile come partner nel Microsoft Partner Network. Tali offerte possono includere microsoft Action Pack, l'appartenenza alle competenze Silver o l'appartenenza a competenze Gold.

Per attivare una sottoscrizione basata su licenza: 

1. Individuare la sottoscrizione specifica nella pagina Azure e prodotti cloud. 

1. Selezionare la freccia a discesa nella colonna Visualizza **dettagli** a destra per la sottoscrizione. 

1. Leggere le istruzioni nella pagina successiva. In questo modo verranno spiegate altre informazioni su come riscattare le chiavi. 
...
1. Quando si è pronti per riscattare la chiave, selezionare l'opzione **Procedura** dettagliata di attivazione disponibile nella parte inferiore di questa pagina.
...
> [!Note]
> Una volta riscattata una chiave, la stessa chiave non può essere riscattata una seconda volta. Si notino anche i punti importanti seguenti:
>
> - Verificare se è necessario aggiungere o meno altre licenze (postazioni), quante e se estendere o meno la data di fine. Se si sceglie di aggiungere altre  licenze (postazioni) e non si estende la data di fine, il sistema fornirà meno di 12 mesi di sottoscrizione attiva. 
>
> - Assicurarsi di aver eseguito l'accesso con le credenziali corrette e di attivare le chiavi nel tenant Azure Active Directory (AAD) corretto. Accedere sempre con una finestra del browser privata/in incognito per assicurarsi di attivare le chiavi nel tenant corretto.
>
> - La data di scadenza del vantaggio visualizzata in Microsoft Partner Center indica l'ultima data per l'esercizio dei diritti di utilizzo per tale vantaggio. Questa data può differire dalla data di fine della sottoscrizione elencata nel portale dell'interfaccia di amministrazione di M365. Ciò può essere dovuto a diversi fattori, ad esempio la data di attivazione del vantaggio, il numero di licenze associate alla chiave e le licenze aggiunte o estese nel portale di amministrazione.
...
Seguire questa procedura per aggiungere altre licenze o estendere la data di fine per una sottoscrizione esistente:

1. Copiare e incollare il collegamento seguente in una finestra del browser privata **o in incognito**.

   [https://admin.microsoft.com/AdminPortal/Home#/subscriptions](https://admin.microsoft.com/AdminPortal/Home#/subscriptions)

1. Quando si accede al portale dell'interfaccia di amministrazione di **M365,** assicurarsi di accedere con il tenant Azure Active Directory corretto. Selezionare quindi la sottoscrizione cloud specifica nell'elenco ed eseguire una delle operazioni seguenti (per rinnovare la sottoscrizione o aggiungere altre licenze):
...
Questa sezione descrive alcuni scenari di licenza di esempio per le sottoscrizioni associate ai prodotti Azure e cloud. In questi scenari si apprenderà come vengono calcolate le date di fine della sottoscrizione.

### <a name="scenario-1"></a>Scenario 1

Se in una sottoscrizione sono attive 100 licenze e si sta tentando di estendere la data di fine usando una chiave con 25 licenze, la data di fine verrà estesa di tre (3) mesi per la sottoscrizione con 100 licenze.

### <a name="scenario-2"></a>Scenario 2

Se in una sottoscrizione sono attive 100 licenze e si sta tentando di estendere la data di fine usando una chiave con 100 licenze, la data di fine verrà estesa di 12 mesi.

### <a name="scenario-3"></a>Scenario 3

Se in una sottoscrizione sono attive cinque (5) licenze e si sta tentando di estendere la data di fine usando una chiave con 10 licenze, la data di fine verrà estesa di 24 mesi per la sottoscrizione con cinque (5) licenze (o postazioni).

### <a name="scenario-4"></a>Scenario 4

Se si sta tentando di estendere la data di fine usando una chiave con più di 3 licenze/postazioni attive, verrà visualizzato il messaggio di errore seguente. Questo messaggio è di progettazione, poiché la data di fine della sottoscrizione può essere estesa di un massimo di 3 anni.
...
- Informazioni sulla [migrazione da Office 365 E3 a Microsoft 365 Business Premium](/microsoft-365/business/migrate-from-e3)
