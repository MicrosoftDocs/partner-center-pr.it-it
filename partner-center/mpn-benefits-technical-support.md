...
**Ruoli appropriati:** amministratore globale | Amministratore partner MPN | Agente amministratore


## <a name="overview"></a>Panoramica

La [pagina Vantaggi](https://partner.microsoft.com/dashboard/mpn/membership/benefits/technical) tecnici mostra informazioni su 2 vantaggi separati

1.  **Eventi imprevisti del supporto tecnico del prodotto: vedere di seguito**
2.  Servizi di prevendita e distribuzione (TPD) tecnici: [altre informazioni](https://docs.microsoft.com/partner-center/technical-benefits)

**Questo articolo è per i membri di Microsoft Partner Network Partner (MPN) che devono sapere come creare un evento imprevisto del supporto tecnico usando i vantaggi dei partner MPN**

## <a name="view-and-activate-your-technical-support-benefits"></a>Visualizzare e attivare i vantaggi del supporto tecnico

Prima di tutto è necessario attivare, quindi vengono visualizzati l'ID di accesso e l'ID contratto, che consentono di associare i vantaggi all'account. L'account viene utilizzato per accedere al portale di supporto tecnico pertinente e creare una richiesta di supporto tecnico.
...
3. Per usare il supporto tecnico Microsoft o il supporto tecnico firma, è necessario attivare il vantaggio. Per attivare il vantaggio, fare clic sul **pulsante** Attiva. L'account utente usato per l'attivazione richiede i ruoli Amministratore globale o Amministratore partner MPN. 

4. L'ID di accesso e l'ID contratto verranno visualizzati a tutti gli utenti dopo l'attivazione. Quando vengono visualizzati l'ID di accesso e l'ID contratto, è possibile usare il supporto tecnico microsoft e il supporto tecnico del prodotto per la firma (membri Gold e Silver).
...
Il supporto tecnico fornisce la risoluzione di un problema specifico, un messaggio di errore o una funzionalità che non funziona come previsto.

> [!NOTE]
> I vantaggi del supporto tecnico devono essere attivati prima di poterli usare.

Come parte dell'appartenenza Microsoft Partner Network, i partner sono idonei per il supporto tecnico seguente
...
È possibile usare **il Supporto tecnico Microsoft** per il supporto tecnico su prodotti locali recenti.

Saranno necessari l'ID di accesso e l'ID contratto la prima volta che si usa il vantaggio. I vantaggi sono associati all'account utente fino alla scadenza o non sono più presenti altri eventi imprevisti di supporto.

Qualsiasi account utente (AAD o MSA) può usare il supporto tecnico per i prodotti locali se ha l'ID di accesso e l'ID contratto

Selezionare [Supporto per le](https://support.serviceshub.microsoft.com/supportforbusiness/create) aziende per creare una richiesta di supporto per un prodotto locale.

## <a name="how-many-partner-support-incidents-are-remaining"></a>Numero di eventi imprevisti di supporto per i partner rimanenti
È possibile ottenere queste informazioni solo dal portale di supporto locale di Support for Business. Sfortunatamente, i portali cloud (ad esempio, Azure, LCS e altri) non visualizzano queste informazioni.
...
Passaggio 1 Aprire [Supporto per le aziende](https://support.serviceshub.microsoft.com/supportforbusiness/create) e selezionare Windows 10 e immettere le informazioni fittizie

Passaggio 2 Se il contratto di supporto è già associato all'account, verranno visualizzati gli eventi imprevisti di supporto. Per il supporto locale, dovrebbe essere visualizzato il contratto di supporto partner (a meno che non sia più valido) e questo include la data di inizio, la data di fine e gli eventi imprevisti rimanenti. Se è necessario associare il contratto di supporto, espandere "È necessario acquistare o aggiungere altri piani di supporto" e fare clic sul pulsante Aggiungi contratto per immettere l'ID di accesso e l'ID contratto. Espandere il nodo "Si vuole visualizzare i piani di supporto non disponibili?" per visualizzare le informazioni sulla firma dei partner. Il supporto per i partner verrà visualizzato qui solo se i contratti sono scaduti, non sono [](https://aka.ms/n-1) rimasti eventi imprevisti o se è stato selezionato un prodotto locale precedente (vedere aka.ms/n-1 per i prodotti di supporto)
...
I partner di competenza Gold o Silver possono **usare** Signature Cloud Support per il supporto tecnico sui prodotti cloud.

Saranno necessari l'ID di accesso e l'ID contratto la prima volta che si usa il vantaggio. I vantaggi sono associati all'account utente fino alla scadenza o non sono rimasti altri eventi imprevisti di supporto.

> [!NOTE]  
> Quando si usa l'oggetto portale di Azure. Se il contratto di supporto è già associato all'account, verrà visualizzato il messaggio di errore "ID di accesso già associato..." quando si aggiungono l'ID di accesso e l'ID contratto. vedere di seguito
...
Quando si usa il portale di Azure, è possibile usare Signature Cloud Support selezionando Guida e supporto, Piani di supporto, quindi "Collegare i vantaggi del supporto" e immettere l'ID di accesso e l'ID contratto.
...
È possibile confrontare [i piani di supporto dei partner](https://partner.microsoft.com/support/partnersupport) o vedere di seguito per i piani specifici dei partner:
...
*   [Prevendite tecniche e servizi di distribuzione (TPD)](https://docs.microsoft.com/partner-center/technical-benefits)
