---
title: Partner Center Insights - Report di CloudAscent Suggerimenti
...
:::image type="content" source="images/pci/cust-oppor-11.png" alt-text="Screenshot del dashboard di riepilogo delle opportunità dei clienti.":::
...
1. **Top Unmanaged include** i principali clienti SMB con la massima opportunità per Microsoft. I clienti principali non gestiti tipici condividono caratteristiche simili agli account gestiti, con un numero elevato di dipendenti, budget e spese IT di grandi dimensioni e grandi quantità di potenziali ricavi per Microsoft.
   ...
   - **Top Unmanaged User Based**:include gli account con 300 o più dipendenti. User-Based account sono ottimi obiettivi per il primo acquisto o l'espansione di prodotti di sottoscrizione basati sull'utente, ad esempio Microsoft 365, Dynamics 365 o Surface.
   - **Top Unmanaged Compute Based** : include gli account con un potenziale di Azure superiore a $ 10.000. Gli account basati su calcolo includono Azure esistente. account con un potenziale anno futuro significativo e account che devono ancora acquistare Azure, ma hanno un potenziale per Azure superiore a $ 10.000.

2. **Medium Business** include clienti esistenti e account prospect con 25-300 dipendenti.

3. **Small Business** include aziende con 10-25 dipendenti.
...
**I sottosegmenti** **principali** di business non gestiti e medi rappresentano i clienti con valore di durata elevata (LTV) per Microsoft e i partner Microsoft. Per questo, sono le principali aree di interesse per la crescita in questo segmento. In questi due sottosegmenti si è posizionati meglio per acquisire il socket con Microsoft 365, monetizzare ulteriormente con le app line-of-business (LOB) D365/Azure e realizzare una rete LTV elevata per Microsoft.

Attualmente sono disponibili due aree chiave di opportunità: 1. il cliente aggiunge crescita; 2. Pur acquisendo i socket cloud leader con Microsoft 365, abbiamo una grande opportunità in Dynamics 365 e Azure.
...
SMB usa la tecnologia di Machine Learning per guidare le stime dei clienti di vendita e marketing all'interno dei segmenti Top Unmanaged e Medium Business. Come vengono raccolti e trasformati i segnali in raccomandazioni sulla propensione?
...
3. Clustering
   ...
   Il clustering consente agli utenti di scegliere come destinazione clienti specifici per le iniziative di vendita e marketing in base a fattori di segmento, ad esempio prodotto, geo, settore e verticale.
...
:::image type="content" source="images/pci/cust-oppor-7.png" alt-text="Modello di tendenza CloudAscent.":::
