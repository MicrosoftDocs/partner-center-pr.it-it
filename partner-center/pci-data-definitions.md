...
Usando l'hub Scarica report nel dashboard informazioni dettagliate, è possibile esportare i set di dati non elaborati. 
...
| PGA_MPNId | Identificatore mpn dell'account globale del partner | 
| PGA_PartnerName | Nome dell'account globale del partner | 
| City | Località del partner | 
| Paese | Località del paese del partner | 
| HierarchyLevel | Indica se si tratta di un ID MPN globale o di un ID MPN di posizione | 
...
| SubscriptionState | Stato della sottoscrizione (attivo o varianza) | 
| Month | Mese per il quale vengono segnalati l'utilizzo e i ricavi | 
...
### <a name="cloud-ascent---microsoft-365-propensity-report"></a>**Cloud Ascent : Microsoft 365 di tendenza**
...
| Area di vendita | Territorio di vendita del cliente a cui viene fatturato il punteggio per laità | 
...
| Abilitare il lavoro remoto - Exchange Online di destinazione | I clienti con una sottoscrizione di Exchange Online attiva possono eseguire l'upselling Microsoft 365 | 
...
| Abilitare Il lavoro remoto - Acquisizione locale (versione EOL) con propensione di Cloud Ascent - +10 licenze | Cliente con un client EOL locale di Office o Windows, ovvero una versione EOL o una versione precedente. Il cliente ha 10 o più licenze. Il cliente ha un punteggio di propensione. Il partner deve essere la destinazione per la conversione in Microsoft 365. | 
| Abilitare il lavoro remoto - acquisizione locale (versione EOL) con la propensione di Cloud Ascent - <10 licenze | Cliente con un client EOL locale di Office o Windows, ovvero una versione EOL o una versione precedente. Il cliente ha meno di 10 licenze. Il cliente ha un punteggio di propensione. Il partner deve essere la destinazione per la conversione in Microsoft 365. | 
| Abilitare Il lavoro remoto - Acquisizione locale (versione EOL) senza propensione di Cloud Ascent - +10 licenze | Cliente con un client Office o Windows locale corrente, ovvero una versione EOL o una versione precedente. Il cliente ha 10 o più licenze. Il cliente non ha un punteggio di propensione. Il partner deve essere la destinazione per la conversione in Microsoft 365. | 
| Abilitare il lavoro remoto - acquisizione locale (versione EOL) senza propensione di Cloud Ascent - <10 licenze | Cliente con un client Office o Windows locale corrente, ovvero una versione EOL o una versione precedente. Il cliente ha meno di 10 licenze. Il cliente non ha un punteggio di propensione. Il partner deve essere la destinazione per la conversione in Microsoft 365. | 
...
| Trasformare la produttività dell'organizzazione - Superficie di attacco | Il cliente mostra una tendenza per Surface | 
| M365Cluster | Identifica la tendenza di un cliente ad acquistare Microsoft 365. Target Act Now and Evaluate clusters because they'll produce higher yield. Target Nurture and Educate customers only if there is still capacity after Act Now and Evaluate customers are targeted. | 
...
| M365Intent | I segnali correlati ai social media e al comportamento online di un cliente definiscono la finalità. L'assegnazione dei punteggi delle finalità viene sovrapposta in Fit per definire i cluster. L'assegnazione dei punteggi delle finalità viene aggiornata mensilmente. | 
...
| SurfaceIntent | I segnali correlati ai social media e al comportamento online di un cliente definiscono finalità. L'assegnazione dei punteggi delle finalità viene sovrapposta in Fit per definire i cluster. L'assegnazione dei punteggi delle finalità viene aggiornata mensilmente. | 
| O365Cluster | Identifica la tendenza del cliente ad acquistare Office 365. Target Act Now e Evaluate clusters perché produrranno un rendimento superiore. Target Nurture and Educate customers only if there is still capacity after Act Now and Evaluate customers are targeted(Target Nurture and Educate customers only if there is still capacity after Act Now and Evaluate customers are targeted). | 
| O365Fit | Punti dati interni ed esterni che definiscono firmographics. L'assegnazione dei punteggi di adattamento usa un modello simile alle migliori piccole e medie imprese per confrontare i clienti e verificare se sono idonei per i prodotti cloud Microsoft. L'assegnazione dei punteggi di adattamento viene aggiornata ogni trimestre. | 
| O365Intent | I segnali correlati ai social media e al comportamento online di un cliente definiscono finalità. L'assegnazione dei punteggi delle finalità viene sovrapposta in Fit per definire i cluster. L'assegnazione dei punteggi delle finalità viene aggiornata mensilmente. | 
...
| IsNonProfit | Indica se l'organizzazione è no profit (Sì o No) | 
| Attivare la vendita digitale - Microsoft 365 - dimensioni della postazione >= 25 postazioni (modello di tendenza SalesPro) | Cliente senza Dynamics 365. Dimensioni postazione: 25+. Il partner deve avere come destinazione il cross-selling di Dynamics 365 SalesPro. | 
| Attivare la vendita digitale - Dynamics 365 SalesPro ( Act Now or Evaluate) | Clienti ad alta tendenza senza Dynamics 365. Il partner deve avere come destinazione Dynamics 365 SalesPro. | 
| Managing Financial Risk & Fraud - Dynamics on-premises install base - Navision (Business Centrality model) | Cliente esistente con Navision locale. Il partner deve avere come destinazione Dynamics 365 Business Central. | 
| Managing Financial Risk & Fraud - Dynamics on-premises install base - Dynamics AX (Dynamics 365 Finance + Operationsity model) | Cliente esistente con AX locale. Il partner deve avere come destinazione Dynamics 365 Finance + Operations. | 
| Managing Financial Risk & Fraud - Dynamics on-premises install base - Great Plains (Business Centrality model) | Cliente esistente con Great Plains locale. Il partner deve essere la destinazione di Dynamics 365 Business Central. | 
...
| Gestione delle frodi & rischio finanziario - Base di installazione locale di Dynamics - Altri (modello di propensione di Business Central) | Cliente esistente con altre soluzioni locali non elencate in precedenza. Il partner deve essere la destinazione di Dynamics 365 Business Central. | 
...
| Creare processi aziendali Agile - Base di competenze dynamics - Mendix/OutSystems/Salesforce (modello di propensione dynamics 365) | Creare processi aziendali agile - Base di competenze dynamics - Mendix/OutSystems/Salesforce (modello di propensione dynamics 365) | 
...
| D365FOIntent | I segnali correlati ai social media e al comportamento online di un cliente definiscono finalità. L'assegnazione dei punteggi delle finalità viene sovrapposta in Fit per definire i cluster. L'assegnazione dei punteggi delle finalità viene aggiornata mensilmente. | 
| D365CECluster | Identifica la propensione del cliente ad acquistare Dynamics 365 Customer Engagement. I clienti che mostrano una propensione per Customer Engagement saranno nelle categorie Medie e Piccole. Target Act Now e Evaluate clusters, perché produrranno un rendimento superiore. Target Nurture and Educate customers only if there is still capacity after you target Act Now and Evaluate customers (Target Nurture and Educate customers only if there is still capacity after you target Act Now and Evaluate customers). | 
...
| DynamicsOnPremAXorCRM_HasOpenRenewal | Identifica se il cliente ha un rinnovo aperto per Dynamics on-premises AX o CRM | 
| M365UpsellCustomer | Identifica se il cliente visualizza la tendenza dell'Microsoft 365 | 
...
### <a name="cloud-ascent---azure-propensity-report"></a>**Cloud Ascent - Report di tendenza di Azure**
...
| Eseguire la migrazione - EOL SQL - EOL SQL Server IB con cloud ascent : oltre 5 licenze | Cliente con un account EOL locale SQL Server,ovvero una versione EOL o precedente. Il cliente ha più di 5 licenze. Il cliente ha un punteggio di tendenza. Il partner deve avere come destinazione questo cliente per la migrazione ad Azure. | 
| Eseguire la migrazione - EOL SQL - EOL SQL Server IB con cloud ascent - <5 licenze | Cliente con un account EOL locale SQL Server,ovvero una versione EOL o precedente. Ha meno di 5 licenze. Cliente che ha un punteggio di tendenza. Il partner deve avere come destinazione questo cliente per la migrazione ad Azure. | 
| Eseguire la migrazione - EOL SQL - EOL SQL Server IB senza cloud Ascent - Oltre 5 licenze | Cliente con un account EOL locale SQL Server,ovvero una versione EOL o precedente. Il cliente ha 5 o più licenze. Il cliente non ha un punteggio di tendenza. Il partner deve avere come destinazione questo cliente per la migrazione ad Azure. | 
| Eseguire la migrazione - EOL SQL - EOL SQL Server IB senza cloud ascent - <5 licenze | Cliente con un account EOL locale SQL Server,ovvero una versione EOL o precedente. Il cliente ha meno di 5 licenze. Il cliente non ha un punteggio di propensione. Il partner deve avere come destinazione questo cliente per la migrazione ad Azure. | 
| Eseguire la migrazione - Eseguire la migrazione di Windows Server locale - Windows Server IB corrente con propensione di Cloud Ascent - Oltre 5 licenze | Cliente che dispone di un Windows Server locale corrente, ovvero una versione successiva a EOL. Il cliente ha più di 5 licenze. Il cliente ha un punteggio di propensione. Il partner deve avere come destinazione questo cliente per la migrazione ad Azure. | 
| Eseguire la migrazione - Eseguire la migrazione di Windows Server locale - Windows Server IB corrente con propensione di Cloud Ascent - <5 licenze | Cliente che dispone di un Windows Server locale corrente, ovvero una versione successiva a EOL. Il cliente ha meno di 5 licenze. Il cliente ha un punteggio di propensione per Azure. Il partner deve avere come destinazione questo cliente per la migrazione ad Azure. | 
| Eseguire la migrazione - Eseguire la migrazione di Windows Server locale - Windows Server IB corrente senza propensione di Cloud Ascent - Oltre 5 licenze | Cliente che dispone di un Windows Server locale corrente, ovvero una versione successiva a EOL. Il cliente ha più di 5 licenze. Il cliente non ha un punteggio di propensione. Il partner deve avere come destinazione questo cliente per la migrazione ad Azure. | 
| Eseguire la migrazione - Eseguire la migrazione di Windows Server locale - Windows Server IB corrente senza propensione di Cloud Ascent - <5 licenze | Cliente che dispone di un Windows Server locale corrente, ovvero una versione successiva a EOL. Il cliente ha meno di 5 licenze. Il cliente non ha un punteggio di tendenza. Il partner deve avere come destinazione questo cliente per la migrazione ad Azure. | 
| Eseguire la migrazione a Azure SQL o a macchine virtuali (VM) SQL - SQL Server IB con cloud ascent - Oltre 5 licenze | Cliente con una versione locale corrente SQL Server,ovvero una versione successiva a EOL. Il cliente ha più di 5 licenze. Il cliente ha un punteggio di tendenza. Il partner deve avere come destinazione questo cliente per la migrazione ad Azure. | 
| Eseguire la migrazione a Azure SQL o SQL - Current SQL Server IB with Cloud Ascentity - <5 licenses | Cliente con una versione locale corrente SQL Server,ovvero una versione successiva a EOL. Il cliente ha meno di 5 licenze. Il cliente ha un punteggio di tendenza. Il partner deve avere come destinazione questo cliente per la migrazione ad Azure. | 
...
| Eseguire la migrazione a macchine Azure SQL o SQL - Versione corrente SQL Server IB senza l'autenticità di Cloud Ascent - <5 licenze | Cliente con una versione locale corrente SQL Server,ovvero una versione successiva a EOL. Il cliente ha meno di 5 licenze. Il cliente non ha un punteggio di propensione. Il partner deve avere come destinazione questo cliente per la migrazione ad Azure. | 
...
| Eseguire la migrazione - Desktop virtuale Windows - Cross Selling Modern Work to Azure/WVD | Identifica i clienti con Microsoft 365 e non dispone di Azure. Il partner deve avere come destinazione questo cliente per la migrazione ad Azure. | 
...
| Innovazione - Analytics - Power BI IB con elevata riservatezza di Azure | Clienti con sottoscrizione e Active Power BI tra cui: Power BI - Standalone Pro, Power BI - Azure Suites, Power BI - Office Suites, Power BI Suites - Microsoft 365 | 
...
| Versione del data center di Windows Server | Visualizza la versione degli acquisti di Windows Data Center da parte del cliente | 
...
| AzureIntent | I segnali correlati ai social media e al comportamento online di un cliente definiscono finalità. L'assegnazione dei punteggi delle finalità viene sovrapposta in Fit per definire i cluster. L'assegnazione dei punteggi delle finalità viene aggiornata mensilmente. | 
...
### <a name="cloud-ascent---agreement-renewal-propensity-report"></a>**Cloud Ascent - Report sulla tendenza al rinnovo del contratto**
...
| Top Unmanaged - User Base | Principali clienti non gestiti : utenti | 
| IsNonProfit | Indica se l'organizzazione è no profit (Sì o No) | 
| Google ha | Identifica se il cliente mostra segnali competitivi per la proprietà di prodotti AWS | 
| Dispone di AWS | Identifica se il cliente mostra segnali competitivi per la proprietà di prodotti AWS | 
| Azure Cluster | Identifica la propensione del cliente ad acquistare Azure. Target Act Now e Evaluate clusters, perché produrranno un rendimento superiore. Target Nurture and Educate customers only if there is still capacity after you target Act Now and Evaluate customers (Target Nurture and Educate customers only if there is still capacity after you target Act Now and Evaluate customers). | 
...
| Microsoft 365 Cluster | Identifica la tendenza del cliente ad acquistare Microsoft 365. Target Act Now e Evaluate clusters, perché produrranno un rendimento più elevato. Target Nurture and Educate customers only if there is still capacity after you target Act Now and Evaluate customers. | 
| Programma di licenza | Identifica il tipo di programma di licenza per il rinnovo | 
| Agreement ID | Identificatore del contratto | 
| Data di fine dell'accordo | Data di fine del contratto | 
| Tipo di scadenza | Tipo di scadenza | 
| Ricavi in scadenza | Ricavi associati alle sottoscrizioni in scadenza | 
| Ha EA | Identifica se un rinnovo è una sottoscrizione EA o EA | 
| Ha aperto | Identifica se un rinnovo è un contratto Open o Open Value | 
| Cliente di Azure Upsell | Identifica se il cliente mostra la propensione all'upsell per Azure | 
| Microsoft 365 cliente Upsell | Identifica se il cliente mostra la propensione all'upsell per Microsoft 365 | 
| RevSumDivisionName | Identifica il prodotto che può essere rinnovato | 
