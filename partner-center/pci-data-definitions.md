...
| RevenueUSD | Ricavi in dollari USA | 
...
| Month | Mese per il quale vengono segnalati l'utilizzo e i ricavi | 
| IsAutoRenew | Indica se la sottoscrizione viene newed (Sì o No) | 
| CustomerName | Nome del cliente | 
...
| ACR_USD | Ricavi consumati (ACR) di Azure in dollari STATUNITENSI | 
...
| Conteggio delle riunioni | Numero di riunioni | 
...
| CompetencyStatus | Stato corrente della competenza (attivo, inattivo o nel periodo di tolleranza) | 
...
| Month | Mese per il quale vengono segnalate le metriche | 
| MetricName | Nome della metrica pertinente per la competenza | 
| MetricaMonthlyContribution | Contributo mensile della metrica | 
...
| Stato | Posizione geografica dello stato dell'organizzazione | 
...
| Abilitare il lavoro remoto - Acquisizione locale (versione corrente) con cloud ascent - +10 licenze | Cliente che dispone di un client Office o Windows locale. Ovvero, la versione del client è successiva a una versione end-of-life (EOL). Il cliente ha 10 o più licenze. Cliente che ha un punteggio di tendenza. Il partner deve essere la destinazione per la conversione Microsoft 365. | 
| Abilitare il lavoro remoto - Acquisizione locale (versione corrente) con l'abilitazione di Cloud Ascent - <10 licenze | Cliente che dispone di un client Office o Windows locale corrente (ovvero una versione successiva a EOL). Il cliente ha meno di 10 licenze. Cliente che ha un punteggio di tendenza. Il partner deve essere la destinazione per la conversione Microsoft 365. | 
| Abilitare il lavoro remoto - Acquisizione locale (versione corrente) senza l'abilitazione di Cloud Ascent - +10 licenze | Cliente che dispone di un client Office o Windows locale corrente (ovvero una versione successiva a EOL). Il cliente ha 10 o più licenze. Il cliente non ha un punteggio di tendenza. Il partner deve essere la destinazione per la conversione Microsoft 365. | 
...
| Abilitare il lavoro remoto - acquisizione locale (versione EOL) senza la riservatezza di Cloud Ascent - <10 licenze | Cliente che dispone di un client Office o Windows locale corrente (ovvero una versione EOL o precedente). Il cliente ha meno di 10 licenze. Il cliente non ha un punteggio di tendenza. Il partner deve essere la destinazione per la conversione Microsoft 365. | 
| Enable Remote Work - high-riservatezza prospect for Microsoft 365 (Act NowithEvaluate) | Cliente potenziale con elevata tendenza per l'Microsoft 365 | 
...
| O365Intent | I segnali correlati ai social media e al comportamento online di un cliente definiscono la finalità. Il punteggio della finalità viene sovrapposto a Fit per definire i cluster. L'assegnazione dei punteggi delle finalità viene aggiornata ogni mese. | 
...
| Ha EA | Identifica se un rinnovo è un contratto Enterprise Agreement (EA) o una sottoscrizione EA | 
| Ha aperto | Identifica se un rinnovo è un contratto Open Value o Open Value | 

### <a name="cloud-ascent---dynamics-365-propensity-report"></a>**Cloud Ascent - Dynamics 365 - Report di tendenza**
...
| Nome partner | Nome del partner | 
| Customer ID | Numero di identificatore del cliente | 
| Numero DUNS | The Dun & Bradstreet number of the customer who's being scored foramoity | 
| Nome account | Nome dell'account | 
| Dominio | Dominio dell'account | 
| Dimensioni dell'organizzazione | Dimensioni dell'organizzazione | 
| Settore | Settore a cui appartiene l'organizzazione | 
| Vertical | La verticale del cliente a cui viene dato un punteggio per laità, come identificato da Microsoft, D&B e altri standard del settore
| Ad aree | Area geografica della località | 
| Affiliata | La filiale del cliente a cui viene in corso l'assegnazione dei punteggi per la continuità | 
...
| Stato | Posizione dello stato geografico | 
| CAP | Codice postale dell'organizzazione | 
| Paese | Località geografica del paese | 
| Segment | Segmento di mercato | 
| Segmento secondario | Sottosegment di mercato | 
| Riepilogo del tipo di SMC | Categorizzazione di un cliente: le principali basi di utenti non gestiti sono clienti con più di 300 dipendenti, le principali basi di calcolo non gestite sono clienti con un potenziale di tre anni di USD 10.000 in Azure, le aziende medie sono clienti con almeno 25 dipendenti e le piccole imprese sono clienti con meno di 25 dipendenti. | 
| Top Unmanaged - Compute Base | Principali clienti non gestiti : calcolo | 
...
| Top Unmanaged - User Base | Principali clienti non gestiti : utenti | 
| IsNonProfit | Indica se l'organizzazione è no profit (Sì o No) | 
| Eseguire la migrazione - EOL Windows Server - EOL Windows Server IB with Cloud Ascent propensity - 5+ licenses | Cliente che dispone di un'istanza di Windows Server locale di EOL, ovvero una versione di EOL o una versione precedente. Il cliente ha 5 o più licenze. Cliente con un punteggio di propensione. Il partner deve avere come destinazione questo cliente per la migrazione ad Azure. | 
| Eseguire la migrazione - EOL Windows Server - EOL Windows Server IB with Cloud Ascent propensity - <5 licenses | Cliente che dispone di un'istanza di Windows Server locale di EOL, ovvero una versione di EOL o una versione precedente. Il cliente ha meno di 5 licenze. Cliente con un punteggio di propensione. Il partner deve avere come destinazione questo cliente per la migrazione ad Azure. | 
| Eseguire la migrazione - EOL Windows Server - EOL Windows Server IB senza propensione di Cloud Ascent - 5+ licenze | Cliente che dispone di un'istanza di Windows Server locale di EOL, ovvero una versione di EOL o una versione precedente. Il cliente ha più di 5 licenze. Il cliente non ha un punteggio di propensione. Il partner deve avere come destinazione questo cliente per la migrazione ad Azure. | 
| Eseguire la migrazione - EOL Windows Server - EOL Windows Server IB senza propensione di Cloud Ascent - <5 licenze | Cliente che dispone di un'istanza di Windows Server locale di EOL, ovvero una versione di EOL o una versione precedente. Ha meno di 5 licenze. Il cliente non ha un punteggio di propensione. Il partner deve avere come destinazione questo cliente per la migrazione ad Azure. | 
...
| Eseguire la migrazione - Desktop virtuale Windows - Cross Sell Modern Work to Azure/WVD | Identifica i clienti con Microsoft 365 e non dispone di Azure. Il partner deve avere come destinazione questo cliente per la migrazione ad Azure. | 
...
| Innovazione - Analytics - Power BI IB con elevata tendenza di Azure | Clienti con sottoscrizione e Active Power BI tra cui: Power BI - Standalone Pro, Power BI - Azure Suites, Power BI - Office Suites, Power BI Suites - Microsoft 365 | 
...
| Ha EA | Identifica se un rinnovo è un contratto EA o una sottoscrizione EA | 
| Ha aperto | Identifica se un rinnovo è un contratto Open Value o Open Value | 
| Cliente di Azure Upsell | Identifica se il cliente viene visualizzato per Azure | 
